"""FastAPI application exposing ORBIT's control-plane and product surface."""
...
from orbit.web import DASHBOARD_HTML
...
def _model_payload(model: Any, app: OrbitApp) -> dict[str, Any]:
...
def create_app(app: OrbitApp | None = None) -> FastAPI:
    ...
    api = FastAPI(title="ORBIT API", version="0.2.0", docs_url="/docs")
    ...
    async def session(request: Request) -> dict[str, Any]:
        ...
        return {"authenticated": True, "state": context.app.state.value, "service": "ORBIT"}
    ...
    async def models(request: Request) -> dict[str, Any]:
        ...
        return {"object": "list", "data": [_model_payload(model, context.app) for model in context.app.model_manager.all()]}
    ...
    async def register_model(payload: ModelRegisterRequest, request: Request) -> dict[str, Any]:
        ...
        if context.app.model_manager is None:
            raise HTTPException(status_code=503, detail="model manager unavailable")
        runtimes = frozenset(payload.runtimes) if payload.runtimes else frozenset(context.app.runtimes.names())
        missing = [name for name in runtimes if context.app.runtimes.get(name) is None]
        if missing:
            raise HTTPException(status_code=422, detail=f"unknown runtime: {', '.join(missing)}")
        spec = ModelSpec(model_id=payload.id, display_name=payload.display_name, modality=payload.modality, size_bytes=payload.size_bytes, min_memory_bytes=payload.min_memory_bytes, capabilities=frozenset(payload.capabilities), runtimes=runtimes, tags=frozenset(payload.tags))
        ...
        return _model_payload(managed, context.app)
    ...
    async def model_detail(model_id: str, request: Request) -> dict[str, Any]:
        ...
        return _model_payload(managed, context.app) | {"error": managed.error, "size_bytes": managed.spec.size_bytes, "modality": managed.spec.modality.value}
    ...
    async def install_model(model_id: str, payload: ModelInstallRequest, request: Request) -> dict[str, Any]:
        ...
        if context.app.router is not None:
            context.app.router.catalog = context.app.models
        return _model_payload(installed, context.app)
    ...
    async def chat_completion(payload: ChatCompletionRequest, http_request: Request) -> Any:
        ...
        try:
            decision = await context.app.router.route(RouteRequest(payload.model, payload.runtime))
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=503, detail=str(exc)) from exc
        generation = GenerationRequest(prompt=prompt, model=decision.model_id, temperature=payload.temperature, max_tokens=payload.max_tokens)
        executor = RuntimeExecutor(decision.plan.runtime)
        if not payload.stream:
            try:
                result = await executor.execute(generation, request_id=http_request.state.request_id)
            except RuntimeExecutionError as exc:
                context.app.request_manager.start(http_request.state.request_id, decision.model_id, decision.runtime_name)
                context.app.request_manager.fail(http_request.state.request_id, str(exc))
            ...
            context.app.request_manager.complete(result.request_id, tokens=token_count)
            metrics_for(http_request).record(latency_ms=result.duration_ms, tokens=token_count)
            return {"id": result.request_id, "object": "chat.completion", "model": decision.model_id, "runtime": decision.runtime_name, "choices": [{"index": 0, "message": {"role": "assistant", "content": result.text}, "finish_reason": "stop"}], "usage": {"completion_ms": round(result.duration_ms, 3), "completion_tokens": token_count}}
        ...
        async def event_stream() -> AsyncIterator[bytes]:
            ...
            try:
                ...
                async for chunk in executor.stream(generation, request_id=request_id):
                ...
                yield _sse({"id": completion_id, "object": "error", "error": {"message": str(exc), "type": "runtime_error"}})
